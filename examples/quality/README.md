...
**route_to_branches**: A conditional edge that inspects the setup result. If `error` is set (for example, repo clone failure), it routes directly to `END`. An empty diff or empty provider matrix routes to `synthesize_and_post`, which reports an appropriate clean or configuration result. Otherwise, it generates one `Send` per (provider, domain) pair and dispatches them to `review_branch`.

**review_branch**: Each branch loads the baseline (if it exists), selects the appropriate prompt (initial vs follow-up), builds an LLM with the branch's provider, binds filesystem + recording tools, and runs the tool loop. Each raw finding receives an opaque source ID used to validate synthesis provenance. Branch failures are isolated: exceptions are caught, partial findings are preserved, and a failed note is appended. Only asyncio.CancelledError propagates (for task cancellation). Graph branches return only reducer-compatible fields (`findings`, `notes`, and `branch_executions`); execution records distinguish successful clean branches from failures during fan-in.

**synthesize_and_post**: Takes the aggregated findings, notes, and branch execution records (concatenated across all branches), uses an LLM with structured output to merge duplicates and consolidate the review, and validates synthesized comments against raw source IDs. Source-less legacy findings receive stable run-local IDs before synthesis. Cited raw findings determine each published comment's location and severity; unrelated cited sources are restored as separate raw comments, and synthesis wording is retained only when it is effectively equivalent to cited reviewer content. The final event has a floor derived from raw finding severities. Untrusted HTML, HTML-comment, and code-fence tokens are neutralized before deterministic provenance footers, and the fully rendered output is redacted before GitHub submission or local artifact creation. A successful publication also saves a baseline for follow-up review.

## Design notes

### Security

**Model output is untrusted.** The synthesis model's verdict (APPROVE/COMMENT/REQUEST_CHANGES) is derived from the PR diff, which is attacker-controlled. Synthesized comments must cite opaque IDs from raw reviewer findings, and the final event cannot be less strict than raw finding severities require. This prevents synthesis from fabricating comments, suppressing a critical finding, or returning a permissive verdict.

**Secrets are redacted after rendering.** The complete artifact, review, or inline comment is passed through `quality._redact.redact_secrets()` after deterministic attribution and provenance are appended. This catches credentials introduced by either model output or generated metadata before publication.
